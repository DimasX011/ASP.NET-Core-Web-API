...
namespace MetricsManager.DAL.Repositories
{
    ...
    public class RamMetricRepository : IRamMetricRepository
    {
        ...
        public GetAllRamMetricsApiRequest GetLastTime()
        {
            GetAllRamMetricsApiRequest dateTimes = new GetAllRamMetricsApiRequest();
            TimeSpan fromTime = TimeSpan.FromSeconds(DateTimeOffset.UtcNow.ToUnixTimeSeconds());
            using (var connection = new SqlConnection(ConnectionString))
            {
                connection.Open();
                SqlCommand command = new SqlCommand("SELECT adress, time FROM rammetrics WHERE time = (SELECT MAX(time) FROM rammetrics WHERE time>@fromTime AND time<@toTime);", connection);
                SqlDataReader reader = command.ExecuteReader();
                while (reader.Read())
                {
                    dateTimes = new GetAllRamMetricsApiRequest { FromTime = fromTime, ToTime = reader.GetTimeSpan(0), ClientBaseAddress = reader.ToString() };
                }
            }
            return dateTimes;
        }
